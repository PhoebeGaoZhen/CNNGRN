...
for net in range(5):  # 这里只考虑第一个网络
    print(network[net] + '正在训练中...................................................................')
    # 1. 读取并转换数据格式
    #     gene10 = 'gene10_'+str(net+1)
    #     gene10_ts =  'gene10_'+str(net+1)+'_ts'
    path = 'DATA\DREAM100\DREAM4_GoldStandard_InSilico_Size100_' + str(net + 1) + '.tsv'
    pathts = 'DATA\DREAM100\insilico_size100_' + str(net + 1) + '_timeseries.tsv'
    ...
    for ki in range(10):
        ...
        for train_index, test_index in kf.split(dataX, labelY):  # 调用split方法切分数据
            ...
            #     row = positionY[i][0]
            #     col = positionY[i][1]
            #     testX[i][0][210 + col] = 0  # 将测试样本Y部分的位置置0
            #     testX[i][0][520 + row] = 0  # 将测试样本Y部分的位置置0
            # print(geneNetwork[row][col]) # 这三个值是一样的，证明找对了
            #                 print(testX[i][0][210+col])
            #                 print(testX[i][0][520+row])
            #                 print('*************************')
            # print(trainX[0].shape)

            # 6.2 创建并训练模型
            model = CNNGRN_definedFunc.create_model_gene100(trainX[0].shape[1])
    ...
    filename = open('D:\\pycharmProjects\\CNNGRN\\results\\0428\\' + network_dict_name + '.txt', 'w')
